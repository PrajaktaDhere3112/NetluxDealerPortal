...
class MyRenewalsAdapter(val context: Context, val ActivationList: List<ActivationDetailsEntity>)
    :RecyclerView.Adapter<MyRenewalsAdapter.viewHolder>(){
    ...
    override fun onBindViewHolder(holder: viewHolder, position: Int) {
        ...
        val expTime = ActivationList[position].expDate.substring(11,16)
        val expDate = ActivationList[position].expDate.substring(0,10)

        val properTime = ProperDate().getProperTime(expTime)
        //setting all the text views and activation state
        holder.tv_name.setText("Name: $name")
        holder.tv_pKey.setText("Exp. Date: $expDate($properTime)")
